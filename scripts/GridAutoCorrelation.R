...
deerWinMoran<-Moran.I(deerWinCR$CR, invDistMatW)
...
DvarioS<-variog(coords = XYmatrixS, data = deerSumCR$CR, breaks = seq(0, 150,15), 
               option = "bin")
plot(DvarioS, xlim = c(0,120), ylim = c(0,15000), main = "Summer")

DvarioW<-variog(coords = XYmatrixW, data = deerWinCR$CR, breaks = seq(0, 150,15), 
                option = "bin")
plot(DvarioW, xlim = c(0,120), ylim = c(0,15000), main = "Winter")

DvarioSp<-variog(coords = XYmatrixSp, data = deerSprCR$CR, breaks = seq(0, 150,15), 
                option = "bin")
plot(DvarioSp, xlim = c(0,120), ylim = c(0,15000), main = "Spring")

DvarioF<-variog(coords = XYmatrixF, data = deerFallCR$CR, breaks = seq(0, 150,15), 
                option = "bin")
plot(DvarioF, xlim = c(0,120), ylim = c(0,15000), main = "Fall")

#################################
#Raccoon Analysis####
#Calculate Moran's I for bear for all Seasons
...
#Need to read more about setting the right bin size and how far we can interpret teh variogram (perhaps only half the field size?)
par(mfrow = c(2,2), mar = c(5,5,5,5))
RvarioS<-variog(coords = XYmatrixS, data = raccoonSumCR$CR, breaks = seq(0, 150,15), 
                option = "bin")
plot(RvarioS, xlim = c(0,120), ylim = c(0,2000), main = "Summer")

RvarioW<-variog(coords = XYmatrixW, data = raccoonWinCR$CR, breaks = seq(0, 150,15), 
                option = "bin")
plot(RvarioW, xlim = c(0,120), ylim = c(0,2000), main = "Winter")

RvarioSp<-variog(coords = XYmatrixSp, data = raccoonSprCR$CR, breaks = seq(0, 150,15), 
                 option = "bin")
plot(RvarioSp, xlim = c(0,120), ylim = c(0,2000), main = "Spring")

RvarioF<-variog(coords = XYmatrixF, data = raccoonFallCR$CR, breaks = seq(0, 150,15), 
                option = "bin")
plot(RvarioF, xlim = c(0,120), ylim = c(0,2000), main = "Fall")

#################################
#Bear Analysis####
#Calculate Moran's I for Raccoon for all Seasons
bearSumMoran<-Moran.I(bearSumCR$CR, invDistMatS)
bearFallMoran<-Moran.I(bearFallCR$CR, invDistMatF)
bearWinMoran<-Moran.I(bearWinCR$CR, invDistMatW)
bearSprMoran<-Moran.I(bearSprCR$CR, invDistMatSp)

#bear Variograms for 4 Seasons. Need to remove the cameras with no data from Fall and Winter to have it work
#Need to read more about setting the right bin size and how far we can interpret teh variogram (perhaps only half the field size?)
par(mfrow = c(2,2), mar = c(5,5,5,5))
BvarioS<-variog(coords = XYmatrixS, data = bearSumCR$CR, breaks = seq(0, 150,15), 
                option = "bin")
plot(BvarioS, xlim = c(0,120), ylim = c(0,300), main = "Summer")

BvarioW<-variog(coords = XYmatrixW, data = bearWinCR$CR, breaks = seq(0, 150,15), 
                option = "bin")
plot(BvarioW, xlim = c(0,120), ylim = c(0,300), main = "Winter")

BvarioSp<-variog(coords = XYmatrixSp, data = bearSprCR$CR, breaks = seq(0, 150,15), 
                 option = "bin")
plot(BvarioSp, xlim = c(0,120), ylim = c(0,300), main = "Spring")

BvarioF<-variog(coords = XYmatrixF, data = bearFallCR$CR, breaks = seq(0, 150,15), 
                option = "bin")
plot(BvarioF, xlim = c(0,120), ylim = c(0,300), main = "Fall")
